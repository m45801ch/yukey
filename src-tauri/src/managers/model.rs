...
impl ModelManager {
    pub fn new(app_handle: &AppHandle) -> Result<Self> {
        ...
        available_models.insert(
            ...
            ModelInfo {
                id: "paraformer-zh".to_string(),
                name: "Paraformer ZH".to_string(),
                description: "精準中文語音辨識（標準版）。".to_string(),
                filename: "paraformer-zh".to_string(),
                url: Some("https://github.com/k2-fsa/sherpa-onnx/releases/download/asr-models/sherpa-onnx-paraformer-zh-2024-03-09.tar.bz2".to_string()),
                sha256: Some("8c6724d0a86bd867217d353db1eaa11f2f143bca446a1f2752e8c551a6f2bde0".to_string()),
                size_mb: 227,
                is_downloaded: false,
                is_downloading: false,
                partial_size: 0,
        ...
        available_models.insert(
            "paraformer-zh-small".to_string(),
            ModelInfo {
                id: "paraformer-zh-small".to_string(),
                name: "Paraformer ZH Small".to_string(),
                description: "快速中文語音辨識（輕量版）。".to_string(),
                filename: "paraformer-zh-small".to_string(),
                ...
                sha256: Some("da92b3db5218c5be53aad53e57d1b6e63e7fc98a0e054fbdd6dbe18e9c6b1450".to_string()),
                size_mb: 80,
                is_downloaded: false,
                is_downloading: false,
                partial_size: 0,
                ...
                supports_translation: false,
                is_recommended: false,
                supported_languages: paraformer_languages,
                supports_language_selection: true,
                is_custom: false,
            },
        );
    ...
    pub async fn download_model(&self, model_id: &str) -> Result<()> {
        ...
        if model_info.is_directory {
            ...
            // Detect archive format (gzip or bzip2) from magic bytes
            let mut file = File::open(&partial_path)?;
            let mut magic = [0u8; 2];
            file.read_exact(&mut magic)?;
            let reader = File::open(&partial_path)?;
            let tar: Box<dyn std::io::Read> = if magic == [0x1f, 0x8b] {
                Box::new(GzDecoder::new(reader))
            } else if magic == [0x42, 0x5a] {
